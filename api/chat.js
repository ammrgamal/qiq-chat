...
export default async function handler(req, res) {
  ...
  const lastRaw = String(messages[messages.length - 1]?.content || '');
    ...
    const systemPrompt = [
      ...
      "- إن كان سؤال المستخدم عن منتج/موديل/سعر/بدائل، استدعِ أداة البحث (searchProducts) بكلمة بحث مناسبة. يجب أن تكون الاقتراحات مبنية على منتجات متوفرة في الكاتالوج (Algolia).",
      "- إذا لم يتوفر المنتج المطلوب بالضبط في الكاتالوج، اقترح بديلًا مناسبًا مع توضيح ذلك: لدينا بديل مناسب. نحن نضيف منتجات وماركات جديدة باستمرار.",
